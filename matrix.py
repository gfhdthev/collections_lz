...
#сумма всех элементов матрицы из условия задания
sum_c = 0
for i in range(N):
    for j in range(M):
        sum_c += C[i][j] #к значению суммы прибавляем каждый элемент матрицы

#сумма всех элементов матрицы, полученной в задании
sum_d = 0
for i in range(N):
    for j in range(M):
        sum_d += D[i][j] #к значению суммы прибавляем каждый элемент матрицы

if sum_c > sum_d:
    print('Матрица, данная в условии, больше')

elif sum_d == sum_c:
    print('Матрицы равны')

else:
    print('Матрица, полученная в задании, больше')